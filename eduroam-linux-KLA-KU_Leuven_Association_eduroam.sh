...
function setup_environment {
  bf=""
  n=""
  ORGANISATION="KU Leuven Association"
  URL="https://admin.kuleuven.be/icts/english/servicedesk"
  SUPPORT="servicedesk@icts.kuleuven.be"
if [ ! -z "$DISPLAY" ] ; then
...
if ! ask "This installer will only work properly if you are a member of ${bf}KU Leuven Association${n} and the user group: ${bf}KU Leuven Association (eduroam).${n}" "Continue" 1 ; then exit; fi
...
class EduroamNMConfigTool:
    ...
    def add_connection(self,ssid):
        server_alt_subject_name_list = dbus.Array({'DNS:radius.kuleuven.be'})
        server_name = 'radius.kuleuven.be'
        if self.nm_version == "0.9" or self.nm_version == "1.0":
    ...
    def main(self):
        ...
        self.delete_existing_connections('campusroam')
        self.add_connection('campusroam')
...
network={
  ...
  identity="${USER_NAME}"
  domain_suffix_match="radius.kuleuven.be"
  phase2="auth=MSCHAPV2"
  password="${PASSWORD}"
}
network={
